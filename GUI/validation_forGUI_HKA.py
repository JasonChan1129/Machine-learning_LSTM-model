...
def run(day=1143,measure_point=1,split_ratio=0.8, timestep=2):
    # FOR REPRODUCIBILITY
    ...
    np.savetxt('dataset_GUI_predictmodel_HKA.csv',dataset, delimiter = ',')
    ...
    model = load_model('HKA_GUI.h5')

    # COMPARING TRAIN AND PREDICTED VALUE
    train_predict = model.predict(trainX)
    train_predict = scaler.inverse_transform(train_predict)
    np.savetxt('train_predict_predictmodel_HKA.csv',train_predict, delimiter = ',')
    ...
    np.savetxt('test_predict_predictmodel_HKA.csv',test_predict, delimiter = ',')
    # testY = scaler.inverse_transform(testY)
    # testPredictout = np.vstack((testY, test_predict))
    # # print('testPredictout:\n', testPredictout)

    future = int((day-1123)/20)
    future_predict = np.zeros((future, timestep, 1))
    future_predict[0:timestep-1, :, :] = test[(len(test)-timestep):]
    ...
    print('futurepredict:\n', futurePredictout)
    np.savetxt('future_predict_predictmodel_HKA.csv',futurePredictout, delimiter = ',')
