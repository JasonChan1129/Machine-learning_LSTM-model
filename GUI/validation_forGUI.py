...
def run(year=5, measure_point=1, split_ratio=0.8, timestep=2):
    # FOR REPRODUCIBILITY
    ...
    np.savetxt('dataset_GUI_predictmodel.csv',dataset, delimiter = ',')
    ...
    model = load_model('LSTM_ave_GUI.h5')

    # COMPARING TRAIN AND PREDICTED VALUE
    train_predict = model.predict(trainX)
    train_predict = scaler.inverse_transform(train_predict)
    np.savetxt('train_predict_predictmodel.csv',train_predict, delimiter = ',')
    ...
    np.savetxt('test_predict_predictmodel.csv',test_predict, delimiter = ',')
    ...
    print('futurepredict:\n', futurePredictout)
    np.savetxt('future_predict_predictmodel.csv',futurePredictout, delimiter = ',')
